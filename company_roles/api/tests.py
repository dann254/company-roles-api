...
from .views import RegistrationView, LoginView, UserListView
...
class UserTest(APITestCase, URLPatternsTestCase):
    ...
    router.register(r'api/auth/register', RegistrationView, basename='register')
    ...
    def test_only_admins_can_view_users(self):
        """ Test fetching all users fails as non admin """
        # Setup the token
        url = reverse('login-list')
    ...
    def test_view_single_user_as_admin(self):
        """ Test fetching one user As admin """
        # Setup the token
        url = reverse('login-list')
        data = {'email': 'admin@example.com', 'password': 'admin1'}
        response = self.client.post(url, data)
    ...
    def test_view_single_user_denied(self):
        """ Test fetching one users fails as non admin """
        # Setup the token
        url = reverse('login-list')
        data = {'email': 'test1@example.com', 'password': 'password1'}
        response = self.client.post(url, data)
    ...
    def test_update_single_users_role_as_admin(self):
        """ Test update one user's role As admin """
        # Setup the token
        url = reverse('login-list')
        data = {'email': 'admin@example.com', 'password': 'admin1'}
        response = self.client.post(url, data)
    ...
    def test_update_single_users_role_denied(self):
        """ Test updating one user's role fails as non admin """
        # Setup the token
        url = reverse('login-list')
        data = {'email': 'test1@example.com', 'password': 'password1'}
        response = self.client.post(url, data)
    ...
    def test_update_admin_role_denied(self):
        """ Test updating admins role fails """
        # Setup the token
        url = reverse('login-list')
        data = {'email': 'test1@example.com', 'password': 'password1'}
        response = self.client.post(url, data)
